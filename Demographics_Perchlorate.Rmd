...
```{r case_counts}
Case_Count <- All_Data %>% 
  summarise("Goiter"=sum(goiter==1, na.rm = T),
...
counts_Gender <- All_Data %>% 
  filter(!is.na(Gender)) %>% 
  group_by(Gender) %>% 
  summarise(count = n(),
            percent = (count/323)*100)

ggplot(counts_Gender, aes(x = "", y = percent, fill = factor(Gender))) +
  geom_col() +
  geom_text(aes(label = round(percent, 1)),
            color = "white",
            size=6,
...
all_demog_complete <- t(All_Data %>%
  filter(ID!="Y1008" & ID!="Y1015" & ID!="R1048" & ID!="R1083" & ID!="C1050" & ID!="C1059" & ID!="C1104" & (!is.na(Cortisol) | !is.na(TSH) | !is.na(fT3) | !is.na(fT4) | !is.na(TotalT4) | !is.na(TotalT3)) & (!is.na(Result_C_mg_dL) | !is.na(Result_P_ng_mL))) %>%
  summarize("n"=length(Site),
            "Male"=sum(Gender==1, na.rm = T),
            "Female"=sum(Gender==2, na.rm = T),
            "Refused Gender"=sum(Gender==3, na.rm = T),
