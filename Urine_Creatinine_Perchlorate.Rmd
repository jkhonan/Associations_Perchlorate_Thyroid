...
NHANES_Perch <- read_xpt("~/Desktop/Yuma_Project/Yuma_R_Project/PERNT_J.XPT") #URXUP8 in ng/mL
NHANES_Demo <- read_xpt("~/Desktop/Yuma_Project/Yuma_R_Project/DEMO_J.XPT")
NHANES_Creat <- read_xpt("~/Desktop/Yuma_Project/Yuma_R_Project/ALB_CR_J.XPT") #UXRUCR in mg/dL
...
geoMean(as.numeric(Sampled_NHANES$URXUP8), na.rm = T) #2.138
...
geoMean(as.numeric(Sampled_All_Data$Result_P_ng_mL), na.rm = T) #2.32
...
geoMean(as.numeric(Sampled_cases$Result_P_ng_mL), na.rm = T) #2.536
...
geoMean(as.numeric(Sampled_NHANES$URXUP8)/(as.numeric(Sampled_NHANES$URXUCR)*10000), na.rm = T) #2.209 2.21
...
### NHANES After Reviewer Feedback
```{r NHANES_Reviewed}
#Libraries
library(haven)
library(survey)
...
#2018 NHANES data files
NHANES_Perch <- read_xpt("~/Desktop/Yuma_Project/Yuma_R_Project/PERNT_J.XPT") #URXUP8 in ng/mL
NHANES_Demo <- read_xpt("~/Desktop/Yuma_Project/Yuma_R_Project/DEMO_J.XPT")
NHANES_Creat <- read_xpt("~/Desktop/Yuma_Project/Yuma_R_Project/ALB_CR_J.XPT")

merged_data_NHANES2018 <- merge(NHANES_Perch, NHANES_Demo, by="SEQN")

adult_data_NHANES <- subset(merged_data_NHANES2018, RIDAGEYR >= 18)

adult_data_NHANES <- subset(adult_data_NHANES, !is.na(adult_data_NHANES$URXUP8))

survey_design <- svydesign(
  id = ~1,
  weights = ~WTSA2YR, #Using the 2 year weights variable
  data = adult_data_NHANES
)

weighted_stats <- svymean(~URXUP8, design = survey_design)
summary(weighted_stats) #Looks a little off so I'm going to manually double check.

weighted_mean <- svytotal(~URXUP8, design = survey_design)
summary(weighted_mean) #Hm

#Going to do manually (n=1791)
weighted_perch <- adult_data_NHANES$WTSA2YR*adult_data_NHANES$URXUP8
sum_weighted_perch <- sum(weighted_perch)
sum_weights <- sum(adult_data_NHANES$WTSA2YR)

weighted_mean <- sum_weighted_perch/sum_weights #3.342
#same value as weighted_stats, so that's good.
adult_data_NHANES_WeightedPerch <- weighted_perch/sum_weights #Only for distribution, not for values.

weighted_variance <- sum(adult_data_NHANES$WTSA2YR * (adult_data_NHANES$URXUP8 - weighted_mean)^2) / sum_weights
weighted_std_dev <- sqrt(weighted_variance) #5.797566

#Median: 
sorted_NHANES_Perch <- adult_data_NHANES[order(adult_data_NHANES$URXUP8), ]

# Calculate cumulative weights and values
cum_weights <- cumsum(sorted_NHANES_Perch$WTSA2YR)
cum_weighted_values <- cumsum(sorted_NHANES_Perch$WTSA2YR * sorted_NHANES_Perch$URXUP8)

# Find the index where cumulative weights exceed half the total
half_total_weights <- sum(sorted_NHANES_Perch$WTSA2YR) / 2
median_index <- which(cum_weights >= half_total_weights)[1]

# The weighted median is the URXUP8 value at this index
weighted_median <- sorted_NHANES_Perch$URXUP8[median_index] #2.34


#geomean = exp(sum(weights*log(perch))/sum(weights))
geomean <- exp(sum(adult_data_NHANES$WTSA2YR*log(adult_data_NHANES$URXUP8))/sum(adult_data_NHANES$WTSA2YR)) #2.197

#geoSD: 
sqrt(exp(sum(adult_data_NHANES$WTSA2YR * ((log(adult_data_NHANES$URXUP8) - log(geomean))^2)) / sum(adult_data_NHANES$WTSA2YR)))

#Now creatinine-corrected perchlorate:
merged_data_NHANES2018_Creat <- merge(merged_data_NHANES2018, NHANES_Creat, by="SEQN")

adult_data_NHANES2018_Creat <- subset(merged_data_NHANES2018_Creat, merged_data_NHANES2018_Creat$RIDAGEYR >= 18)

adult_data_NHANES2018_Creat <- subset(adult_data_NHANES2018_Creat, !is.na(adult_data_NHANES2018_Creat$URXUP8)) #No missing perch

adult_data_NHANES2018_Creat <- subset(adult_data_NHANES2018_Creat, !is.na(adult_data_NHANES2018_Creat$URXUCR)) #No missing creat

adult_data_NHANES2018_Creat$Creat_Corr_Perch <- adult_data_NHANES2018_Creat$URXUP8/(adult_data_NHANES2018_Creat$URXUCR*10000)

#Going to do manually (n=1790) for creatinine corrected perch.
weighted_perch_creat <- adult_data_NHANES2018_Creat$WTSA2YR*adult_data_NHANES2018_Creat$Creat_Corr_Perch
sum_weighted_perch_creat <- sum(weighted_perch_creat)
sum_weights_creat <- sum(adult_data_NHANES2018_Creat$WTSA2YR)

weighted_mean_perch_creat <- sum_weighted_perch_creat/sum_weights_creat #3.3381e-06

weighted_variance_perch_creat <- sum(adult_data_NHANES2018_Creat$WTSA2YR * (adult_data_NHANES2018_Creat$URXUP8 - weighted_mean_perch_creat)^2) / sum_weights_creat
weighted_std_dev_perch_creat <- sqrt(weighted_variance_perch_creat) #6.692114

#geomean = exp(sum(weights*log(perch))/sum(weights))
geomean_creat <- exp(sum(adult_data_NHANES2018_Creat$WTSA2YR*log(adult_data_NHANES2018_Creat$Creat_Corr_Perch))/sum(adult_data_NHANES2018_Creat$WTSA2YR)) #2.326121e-06

#geoSD: 
sqrt(exp(sum(adult_data_NHANES2018_Creat$WTSA2YR*((log(adult_data_NHANES2018_Creat$Creat_Corr_Perch)-log(geomean_creat))^2))/sum(adult_data_NHANES2018_Creat$WTSA2YR))) #1.3287

#Forgot to do creatinine, so coming back here to finish that...

#Going to do manually (n=1790) for creatinine corrected perch.
weighted_creat <- adult_data_NHANES2018_Creat$WTSA2YR*adult_data_NHANES2018_Creat$URXUCR
sum_weighted_creat <- sum(weighted_creat)
sum_weights_creat <- sum(adult_data_NHANES2018_Creat$WTSA2YR)

weighted_mean_creat <- sum_weighted_creat/sum_weights_creat #119.2638

weighted_variance_creat <- sum(adult_data_NHANES2018_Creat$WTSA2YR * (adult_data_NHANES2018_Creat$URXUCR - weighted_mean_creat)^2) / sum_weights_creat
weighted_std_dev_creat <- sqrt(weighted_variance_creat) #78.08952

#geomean = exp(sum(weights*log(perch))/sum(weights))
geomean_creat <- exp(sum(adult_data_NHANES2018_Creat$WTSA2YR*log(adult_data_NHANES2018_Creat$URXUCR))/sum(adult_data_NHANES2018_Creat$WTSA2YR)) #94.47658

#geoSD: 
sqrt(exp(sum(adult_data_NHANES2018_Creat$WTSA2YR*((log(adult_data_NHANES2018_Creat$URXUCR)-log(geomean_creat))^2))/sum(adult_data_NHANES2018_Creat$WTSA2YR))) #1.306614
...
hline_data_A <- data.frame(
  yintercept = c(15, 3.342, 3.095675),
  label = c("Interim Health Advisory Limit", "National Average", "Sample Average"),
  color = c("blue", "firebrick", "forestgreen")
)

tiff("Perchlorate_NHANES_Graph_Review_Update.tiff", units="in", width=10, height=5, res=300) #Only to save in high def.
ggplot(All_Data) +
  geom_point(aes(x = Sample_ID, y = Result_P_ng_mL, shape = as.factor(Site)), size = 2) +
  scale_shape_discrete(labels = c("1" = "CSF", "2" = "RCBH", "3" = "YRMC")) +
  geom_hline(data = hline_data_A, aes(yintercept = yintercept, linetype = label, color = label), size = 1.2) +
...
tiff("Perchlorate_NHANES_LogTrans_Graph_Review_Update.tiff", units="in", width=10, height=5, res=300) #Only to save in high def.
ggplot(All_Data) +
  geom_point(aes(x = Sample_ID, y = Result_P_ng_mL, shape = as.factor(Site), fill = as.factor(Site)), size = 2)+
  scale_y_continuous(trans='log10')+
  scale_shape_manual(name = "Recruitment Site",
...
hline_data3 <- data.frame(
  yintercept = c(2.33, 2.56),
  label = c("National Average", "Sample Average"),
  color = c("firebrick", "forestgreen")
)

tiff("Perch_Creat_NHANES_LogTrans_Graph_Review_Updated.tiff", units="in", width=10, height=7, res=300) #Only to save in high def.
ggplot(All_Data) +
  geom_point(aes(x = Sample_ID, y = Result_P_ug_mL/Result_C_g_mL, shape = as.factor(Site), fill = as.factor(Site)), size = 2)+
  scale_shape_manual(name = "Recruitment Site",
                     values = c(15, 2, 21),
...
tiff("Perch_Creat_NHANES_LogTrans_Graph2_Review_Update.tiff", units="in", width=10, height=7, res=300) #Only to save in high def.
ggplot(All_Data, aes(y = Result_P_ug_mL/Result_C_g_mL, x=as.factor(Site))) +
    geom_violin(color='#ffffff')+
  geom_boxplot()+
  geom_sina(alpha=0.2)+
...
#Stats tests for weighted values...
#t.test(x=log10(All_Data$Result_P_ng_mL), mu=mean(log10(NHANES$URXUP8), na.rm = T), conf.level = 0.95) - this was the original to base off of
...
weighted_se <- sqrt(weighted_variance / sum_weights)

sample_mean <- mean(All_Data$Result_P_ng_mL, na.rm=T)
sample_se <- sqrt(var(All_Data$Result_P_ng_mL, na.rm=T) / sum(!is.na(All_Data$Result_P_ng_mL)))

t_stat <- (sample_mean - weighted_mean) / 
          sqrt(sample_se^2 + weighted_se^2)

df <- (sample_se^2 + weighted_se^2)^2 / 
      (sample_se^4 / (295^2 * (295 - 1)) + 
       weighted_se^4 / sum_weights)

p_value <- 2 * pt(abs(t_stat), df = df, lower.tail = FALSE) #0.1076875
...
#Didn't do earlier so doing now.
weighted_mean_creat <- adult_data_NHANES2018_Creat$WTSA2YR*adult_data_NHANES2018_Creat$URXUCR
sum_weighted_creat <- sum(weighted_mean_creat)
sum_weights_creat <- sum(adult_data_NHANES2018_Creat$WTSA2YR)

weighted_mean_creat <- sum_weighted_creat/sum_weights_creat
weighted_variance_creat <- sum(adult_data_NHANES2018_Creat$WTSA2YR * (adult_data_NHANES2018_Creat$URXUCR - weighted_mean_creat)^2) / sum_weights_creat
weighted_std_dev_creat <- sqrt(weighted_variance_creat)
sum_weights


weighted_se_creat <- sqrt(weighted_variance_creat / sum_weights)

sample_mean_creat <- mean(All_Data$Result_C_mg_dL, na.rm=T)
sample_se_creat <- sqrt(var(All_Data$Result_C_mg_dL, na.rm=T) / sum(!is.na(All_Data$Result_C_mg_dL)))

t_stat_creat <- (sample_mean_creat - weighted_mean_creat) / 
          sqrt(sample_se_creat^2 + weighted_se_creat^2)

df_creat <- (sample_se_creat^2 + weighted_se_creat^2)^2 / 
      (sample_se_creat^4 / (294^2 * (294 - 1)) + 
       weighted_se_creat^4 / sum_weights)

p_value_creat <- 2 * pt(abs(t_stat_creat), df = df_creat, lower.tail = FALSE) #0.8945176

#Finally for creatinine-corrected perchlorate:
#t.test(x=log10(All_Data$Result_P_ng_mL/All_Data$Result_C_ng_mL), mu=mean(log10(NHANES$URXUP8/NHANES$URXUCR), na.rm = T), conf.level = 0.95) - reference

weighted_perch_creat <- adult_data_NHANES2018_Creat$WTSA2YR*adult_data_NHANES2018_Creat$Creat_Corr_Perch
sum_weighted_perch_creat <- sum(weighted_perch_creat)
sum_weights_perch_creat <- sum(adult_data_NHANES2018_Creat$WTSA2YR)

weighted_mean_perch_creat <- sum_weighted_perch_creat/sum_weights_perch_creat #3.3381e-06

weighted_variance_perch_creat <- sum(adult_data_NHANES$WTSA2YR * (adult_data_NHANES$URXUP8 - weighted_mean_perch_creat)^2) / sum_weights_perch_creat
weighted_std_dev_perch_creat <- sqrt(weighted_variance_perch_creat) #6.692124



weighted_se_perch_creat <- sqrt(weighted_variance_perch_creat / sum_weights_perch_creat)


adult_data_NHANES2018_Creat$Creat_Corr_Perch <- adult_data_NHANES2018_Creat$URXUP8/(adult_data_NHANES2018_Creat$URXUCR*10000)


sample_mean_perch_creat <- mean(All_Data$Result_P_ng_mL/(All_Data$Result_C_mg_dL*10000), na.rm=T)
sample_se_perch_creat <- sqrt(var(All_Data$Result_P_ng_mL/(All_Data$Result_C_mg_dL*10000), na.rm=T) / sum(!is.na(All_Data$Result_C_mg_dL)))

t_stat_perch_creat <- (sample_mean_perch_creat - weighted_mean_perch_creat) / 
          sqrt(sample_se_perch_creat^2 + weighted_se_perch_creat^2)

df_perch_creat <- (sample_se_perch_creat^2 + weighted_se_perch_creat^2)^2 / 
      (sample_se_perch_creat^4 / (294^2 * (294 - 1)) + 
       weighted_se_perch_creat^4 / sum_weights) #sum_weights all the same

p_value_perch_creat <- 2 * pt(abs(t_stat_perch_creat), df = df_perch_creat, lower.tail = FALSE) #0.9996575

#Hispanic Subpopulation

adult_data_NHANES2018_Creat$Ethnicity <- ifelse(adult_data_NHANES2018_Creat$RIDRETH1==1|
                             adult_data_NHANES2018_Creat$RIDRETH1==2, 1,
                           ifelse(adult_data_NHANES2018_Creat$RIDRETH1==3|
                                    adult_data_NHANES2018_Creat$RIDRETH1==4, 2, 3)) #1 = Hispanic, 2 = Non-Hispanic, 3 = Don't know (Matches Yuma)

AdultHispanic_NHANES <- subset(adult_data_NHANES2018_Creat, NHANES$Ethnicity==1) #427 observations

AdultHispanic_NHANES <- subset(AdultHispanic_NHANES, !is.na(AdultHispanic_NHANES$URXUP8)) #No missing perch = 412

AdultHispanic_NHANES <- subset(AdultHispanic_NHANES, !is.na(AdultHispanic_NHANES$URXUCR)) #No missing creat = 412

#Going to do manually (n=412)
weighted_perch <- AdultHispanic_NHANES$WTSA2YR*AdultHispanic_NHANES$URXUP8
sum_weighted_perch <- sum(weighted_perch)
sum_weights <- sum(AdultHispanic_NHANES$WTSA2YR)

weighted_mean <- sum_weighted_perch/sum_weights #3.310768
#same value as weighted_stats, so that's good.
#AdultHispanic_NHANES_WeightedPerch <- weighted_perch/sum_weights #Only for distribution, not for values.

weighted_variance <- sum(AdultHispanic_NHANES$WTSA2YR * (AdultHispanic_NHANES$URXUP8 - weighted_mean)^2) / sum_weights
weighted_std_dev <- sqrt(weighted_variance) #5.234052

#Median: 
sorted_NHANES_Perch <- AdultHispanic_NHANES[order(AdultHispanic_NHANES$URXUP8), ]
...
#geomean = exp(sum(weights*log(perch))/sum(weights))
geomean_perch <- exp(sum(AdultHispanic_NHANES$WTSA2YR*log(AdultHispanic_NHANES$URXUP8))/sum(AdultHispanic_NHANES$WTSA2YR)) #2.31471

#geoSD: 
geosd_perch <- sqrt(exp(sum(AdultHispanic_NHANES$WTSA2YR * ((log(AdultHispanic_NHANES$URXUP8) - log(geomean))^2)) / sum(AdultHispanic_NHANES$WTSA2YR))) #1.410984

#Now creatinine-corrected perchlorate:
#merged_data_NHANES2018_Creat <- merge(AdultHispanic_NHANES, NHANES_Creat, by="SEQN")
...
#Going to do manually (n=412) for creatinine corrected perch.
weighted_perch_creat <- AdultHispanic_NHANES$WTSA2YR*AdultHispanic_NHANES$Creat_Corr_Perch
sum_weighted_perch_creat <- sum(weighted_perch_creat)
sum_weights_perch_creat <- sum(AdultHispanic_NHANES$WTSA2YR)

weighted_mean_perch_creat <- sum_weighted_perch_creat/sum_weights_creat #3.154574e-06

weighted_variance_perch_creat <- sum(AdultHispanic_NHANES$WTSA2YR * (AdultHispanic_NHANES$URXUP8 - weighted_mean_perch_creat)^2) / sum_weights_creat
weighted_std_dev_perch_creat <- sqrt(weighted_variance_perch_creat) #6.19326

#geomean = exp(sum(weights*log(perch))/sum(weights))
geomean_perch_creat <- exp(sum(AdultHispanic_NHANES$WTSA2YR*log(AdultHispanic_NHANES$Creat_Corr_Perch))/sum(AdultHispanic_NHANES$WTSA2YR)) #2.370437e-06

#geoSD: 
geosd_perch_creat <- sqrt(exp(sum(AdultHispanic_NHANES$WTSA2YR*((log(AdultHispanic_NHANES$Creat_Corr_Perch)-log(geomean_creat))^2))/sum(AdultHispanic_NHANES$WTSA2YR))) #1.279388

#Going to do manually (n=412) for creatinine corrected perch.
weighted_creat <- AdultHispanic_NHANES$WTSA2YR*AdultHispanic_NHANES$URXUCR
sum_weighted_creat <- sum(weighted_creat)
sum_weights_creat <- sum(AdultHispanic_NHANES$WTSA2YR)

weighted_mean_creat <- sum_weighted_creat/sum_weights_creat #122.7666

weighted_variance_creat <- sum(AdultHispanic_NHANES$WTSA2YR * (AdultHispanic_NHANES$URXUCR - weighted_mean_creat)^2) / sum_weights_creat
weighted_std_dev_creat <- sqrt(weighted_variance_creat) #81.90321

#geomean = exp(sum(weights*log(perch))/sum(weights))
geomean_creat <- exp(sum(AdultHispanic_NHANES$WTSA2YR*log(AdultHispanic_NHANES$URXUCR))/sum(AdultHispanic_NHANES$WTSA2YR)) #97.64905

#geoSD: 
sqrt(exp(sum(AdultHispanic_NHANES$WTSA2YR*((log(AdultHispanic_NHANES$URXUCR)-log(geomean_creat))^2))/sum(AdultHispanic_NHANES$WTSA2YR))) #1.289995


#Hispanic Viz
hline_data_M <- data.frame(
  yintercept = c(15, 3.31, 3.095675),
  label = c("Interim Health Advisory Limit", "Hispanic/Latino Weighted National Average", "Hispanic/Latino Sample Average"),
  color = c("blue", "firebrick", "forestgreen")
)


tiff("Perchlorate_NHANES_Graph_MatchedDemog_Reviewer_Update.tiff", units="in", width=10, height=5, res=300) #Only to save in high def.
ggplot(All_Data) +
  geom_point(aes(x = Sample_ID, y = Result_P_ng_mL, shape = as.factor(Site), fill = as.factor(Site)), size = 2) +
  scale_shape_manual(name = "Recruitment Site",
                     values = c(15, 2, 21),
  ...
  scale_linetype_manual(
    name = "Perchlorate Levels",
    values = c(1, 2, 5),
    labels = c("Interim Health Advisory Limit", "Hispanic/Latino Weighted National Average", "Hispanic/Latino Sample Average")) +
  scale_color_manual(name = "Perchlorate Levels",
                     values = setNames(hline_data_M$color, hline_data_M$label)) +
  guides(linetype = guide_legend(title = "Perchlorate Levels"),
         color = guide_legend(title = "Perchlorate Levels"),
...
hline_data2_M <- data.frame(
  yintercept = c(15, 2.31471, 2.322191), #Replaced 2.138199 with weighted geom average for P8, 2.31471
  label = c("EPA Health Advisory Limit", "Hispanic/Latino Weighted National Average", "Hispanic/Latino Sample Average"),
  color = c("blue", "firebrick", "forestgreen")
)

tiff("Perchlorate_NHANES_Hisp_LogTrans_Graph_Reviewer_Update.tiff", units="in", width=10, height=5, res=300) #Only to save in high def.
ggplot(All_Data) +
  geom_point(aes(x = Sample_ID, y = Result_P_ng_mL, shape = as.factor(Site), fill = as.factor(Site)), size = 2) +
  scale_shape_manual(name = "Recruitment Site",
                     values = c(15, 2, 21),
  ...
  scale_linetype_manual(
    name = "Perchlorate Levels",
    values = c(1, 2, 5),
    labels = c("EPA Health Advisory Limit", "Hispanic/Latino Weighted National Average", "Hispanic/Latino Sample Average")) +
  scale_color_manual(name = "Perchlorate Levels",
                     values = setNames(hline_data2_M$color, hline_data2_M$label)) +
  guides(linetype = guide_legend(title = "Perchlorate Levels"),
         color = guide_legend(title = "Perchlorate Levels"),
...
hline_data3_M <- data.frame(
  yintercept = c(2.370, 2.52), #Geomean replaced with weighted geomean
  label = c("Hispanic/Latino Weighted National Average", "Hispanic/Latino Sample Average"),
  color = c("firebrick", "forestgreen")
)

tiff("Perch_Creat_NHANES_Hisp__LogTrans_Graph_Reviewer_Update.tiff", units="in", width=10, height=7, res=300) #Only to save in high def.
ggplot(Sampled_All_Data) +
  geom_point(aes(x = Sample_ID, y = Result_P_ng_mL/Result_C_ng_mL, shape = as.factor(Site), fill = as.factor(Site)), size = 2)+
  scale_shape_manual(name = "Recruitment Site",
                       values = c(15, 2, 21),
  ...
  scale_linetype_manual(name = "Perchlorate Levels",
                        values = c(2, 5),
                        labels = c("Hispanic/Latino Weighted National Average", "Hispanic/Latino Sample Average"))+
  scale_color_manual(name = "Perchlorate Levels",
                     values = setNames(hline_data3_M$color, hline_data3_M$label))+
  guides(linetype = guide_legend(title = "Perchlorate Levels"),
         color = guide_legend(title = "Perchlorate Levels"),
...
hline_data3_M <- data.frame(
  yintercept = c(2.370, 2.52), 
  label = c("Hispanic/Latino Weighted National Average", "Hispanic/Latino Sample Average"),
  color = c("firebrick", "forestgreen")
)

tiff("Perch_Creat_NHANES_Hisp__LogTrans_Graph2_Reviewer_Update.tiff", units="in", width=10, height=7, res=300) #Only to save in high def.
ggplot(Sampled_All_Data, aes(y = Result_P_ug_mL/Result_C_g_mL, x=as.factor(Site))) +
    geom_violin(color='#ffffff')+
  geom_boxplot()+
  geom_sina(alpha=0.2)+
...
sample_mean <- mean(Sampled_All_Data$Result_P_ng_mL, na.rm=T)
sample_se <- sqrt(var(Sampled_All_Data$Result_P_ng_mL, na.rm=T) / sum(!is.na(Sampled_All_Data$Result_P_ng_mL)))
...
p_value <- 2 * pt(abs(t_stat), df = df, lower.tail = FALSE) #0.4675781

#Creatinine
#t.test(x=log10(Sampled_All_Data$Result_C_mg_dL), mu=mean(log10(NHANES$URXUCR), na.rm = T), conf.level = 0.95)

#URXUCR units mg/dL

#Didn't do earlier so doing now.
weighted_mean_creat <- AdultHispanic_NHANES$WTSA2YR*AdultHispanic_NHANES$URXUCR
sum_weighted_creat <- sum(weighted_mean_creat)
sum_weights_creat <- sum(AdultHispanic_NHANES$WTSA2YR)
...
sample_mean_creat <- mean(Sampled_All_Data$Result_C_mg_dL, na.rm=T)
sample_se_creat <- sqrt(var(Sampled_All_Data$Result_C_mg_dL, na.rm=T) / sum(!is.na(Sampled_All_Data$Result_C_mg_dL)))
...
p_value_creat <- 2 * pt(abs(t_stat_creat), df = df_creat, lower.tail = FALSE) #0.7823231

#Finally for creatinine-corrected perchlorate:
#t.test(x=log10(Sampled_All_Data$Result_P_ng_mL/Sampled_All_Data$Result_C_ng_mL), mu=mean(log10(NHANES$URXUP8/NHANES$URXUCR), na.rm = T), conf.level = 0.95) - reference

weighted_perch_creat <- AdultHispanic_NHANES$WTSA2YR*AdultHispanic_NHANES$Creat_Corr_Perch
sum_weighted_perch_creat <- sum(weighted_perch_creat)
sum_weights_perch_creat <- sum(AdultHispanic_NHANES$WTSA2YR)

weighted_mean_perch_creat <- sum_weighted_perch_creat/sum_weights_perch_creat #3.154574e-06

weighted_variance_perch_creat <- sum(AdultHispanic_NHANES$WTSA2YR * (AdultHispanic_NHANES$URXUP8 - weighted_mean_perch_creat)^2) / sum_weights_perch_creat
weighted_std_dev_perch_creat <- sqrt(weighted_variance_perch_creat) #6.19326
...
sample_mean_perch_creat <- mean(Sampled_All_Data$Result_P_ng_mL/(Sampled_All_Data$Result_C_mg_dL*10000), na.rm=T)
sample_se_perch_creat <- sqrt(var(Sampled_All_Data$Result_P_ng_mL/(Sampled_All_Data$Result_C_mg_dL*10000), na.rm=T) / sum(!is.na(Sampled_All_Data$Result_C_mg_dL)))
...
p_value_perch_creat <- 2 * pt(abs(t_stat_perch_creat), df = df_perch_creat, lower.tail = FALSE) #0.9999783
...
In a case control study design, the outcome analyzed needs to be the outcome that was recruited. So if the study design recruited people with thyroid issues and you include all of them, then the outcome in the models needs to be the general “thyroid issues”. If you choose to specify hypothyroidism as an outcome (which is fine and in some ways makes more sense), then the models need to exclude everyone with hyperthyroidism and other thyroid issues, and the tables/n’s need to reflect hypothyroidism and the appropriate controls. Basically the main model needs to match the study design. If you want, you can conduct a multinomial logistic regression where each level is a different outcome (e.g., controls =0, hypothyroidism = 1, hyperthyroidism =2, other issues= 3). Otherwise I’d just analyze everyone together and then do a sensitivity analysis that is your hypothyroidism models.

Because of this cc study design, you also can’t analyze the other outcomes that were not recruited on (e.g., weight) unless you stratify the analyses by case and control status. This is because you have over enrolled for people with thyroid issues who are more likely to have weight issues.  

If you have thyroid hormone levels on everyone, why don’t you include this as an outcome? We talked about this but I don’t remember what the answer was.For this you can either define your cases/controls based on thyroid hormone levels (seems more sensitive than self report or ICD codes), or you can just run thyroid as a continuous outcome, and need to control for case/control status in the model."
...
#Total T3 (ng/mL)
#total T3 is 80–180 ng/dL = 0.8-1.8 ng/mL
...
#Free T4 (pg/mL)
#free T4 is 0.8–1.8 ng/dL = 8-18 pg/mL
...
#Total T4 (ng/mL)
#total T4 is 5–12 μg/dL = 50-120 ng/mL
