...
All_Data %>% 
  ...
  summarise(`Number Part. with Perchlorate`=length(unique(Sample_ID)))

All_Data %>% 
  filter(!is.na(Result_C_mg_dL)) %>% 
  summarise(`Number Part. with Creatinine`=length(unique(Sample_ID)))
...
All_Data %>% 
  filter(!is.na(fT3)) %>% 
  summarise(`Number Part. with fT3`=length(unique(ID)))

All_Data %>% 
  filter(!is.na(TotalT3)) %>% 
  summarise(`Number Part. with TT3`=length(unique(ID)))

All_Data %>% 
  filter(!is.na(fT4)) %>% 
  summarise(`Number Part. with fT4`=length(unique(ID)))

All_Data %>% 
  filter(!is.na(TotalT4)) %>% 
  summarise(`Number Part. with TT4`=length(unique(ID)))

All_Data %>% 
  filter(!is.na(TSH)) %>% 
  summarise(`Number Part. with TSH`=length(unique(ID)))

All_Data %>% 
  filter(!is.na(Cortisol)) %>% 
  summarise(`Number Part. with Cortisol`=length(unique(ID)))
...
merged_data_NHANES2018 <- merge(merged_data_NHANES2018, NHANES_Creat, by="SEQN")
adult_data_NHANES <- subset(merged_data_NHANES2018, RIDAGEYR >= 18)

adult_data_NHANES$Ethnicity <- ifelse(adult_data_NHANES$RIDRETH1==1|
                             adult_data_NHANES$RIDRETH1==2, 1,
                           ifelse(adult_data_NHANES$RIDRETH1==3|
                                    adult_data_NHANES$RIDRETH1==4, 2, 3)) #1 = Hispanic, 2 = Non-Hispanic, 3 = Don't know (Matches Yuma)


sum(!is.na(adult_data_NHANES$URXUP8))
sum(!is.na(adult_data_NHANES$URXUCR))

adult_data_NHANES_PERCH <- subset(adult_data_NHANES, !is.na(adult_data_NHANES$URXUP8))
adult_data_NHANES_CREAT <- subset(adult_data_NHANES, !is.na(adult_data_NHANES$URXUCR))


survey_design <- svydesign(
  id = ~1,
  weights = ~WTSA2YR, #Using the 2 year weights variable
  data = adult_data_NHANES_PERCH
)
...
weighted_perch <- adult_data_NHANES_PERCH$WTSA2YR*adult_data_NHANES_PERCH$URXUP8
sum_weighted_perch <- sum(weighted_perch)
sum_weights <- sum(adult_data_NHANES_PERCH$WTSA2YR)
...
weighted_variance <- sum(adult_data_NHANES_PERCH$WTSA2YR * (adult_data_NHANES_PERCH$URXUP8 - weighted_mean)^2) / sum_weights
weighted_std_dev <- sqrt(weighted_variance) #5.797566

#Median: 
sorted_NHANES_Perch <- adult_data_NHANES_PERCH[order(adult_data_NHANES_PERCH$URXUP8), ]
median(sorted_NHANES_Perch$URXUP8) #2.22
...
geomean <- exp(sum(adult_data_NHANES_PERCH$WTSA2YR*log(adult_data_NHANES_PERCH$URXUP8))/sum(adult_data_NHANES_PERCH$WTSA2YR)) #2.197

#geoSD: 
sqrt(exp(sum(adult_data_NHANES_PERCH$WTSA2YR * ((log(adult_data_NHANES_PERCH$URXUP8) - log(geomean))^2)) / sum(adult_data_NHANES_PERCH$WTSA2YR))) #1.477

#Creatinine

#Going to do manually (n=1809) for creatinine corrected perch.
weighted_creat <- adult_data_NHANES_CREAT$WTSA2YR*adult_data_NHANES_CREAT$URXUCR
sum_weighted_creat <- sum(weighted_creat)
sum_weights_creat <- sum(adult_data_NHANES_CREAT$WTSA2YR)

weighted_mean_creat <- sum_weighted_creat/sum_weights_creat #121.1058

weighted_variance_creat <- sum(adult_data_NHANES_CREAT$WTSA2YR * (adult_data_NHANES_CREAT$URXUCR - weighted_mean_creat)^2) / sum_weights_creat
weighted_std_dev_creat <- sqrt(weighted_variance_creat) #80.87096

#geomean = exp(sum(weights*log(perch))/sum(weights))
geomean_creat <- exp(sum(adult_data_NHANES_CREAT$WTSA2YR*log(adult_data_NHANES_CREAT$URXUCR))/sum(adult_data_NHANES_CREAT$WTSA2YR)) #95.55992

#geoSD: 
sqrt(exp(sum(adult_data_NHANES_CREAT$WTSA2YR*((log(adult_data_NHANES_CREAT$URXUCR)-log(geomean_creat))^2))/sum(adult_data_NHANES_CREAT$WTSA2YR))) #1.306614


#Now creatinine-corrected perchlorate:
#merged_data_NHANES2018_Creat <- merge(merged_data_NHANES2018, NHANES_Creat, by="SEQN")

adult_data_NHANES_PERCH_CREAT <- subset(adult_data_NHANES_PERCH, !is.na(adult_data_NHANES_PERCH$URXUCR)) #1790
adult_data_NHANES_PERCH_CREAT$Creat_Corr_Perch <- adult_data_NHANES_PERCH_CREAT$URXUP8/(adult_data_NHANES_PERCH_CREAT$URXUCR*10000)

#Going to do manually (n=1790) for creatinine corrected perch.
weighted_perch_creat <- adult_data_NHANES_PERCH_CREAT$WTSA2YR*adult_data_NHANES_PERCH_CREAT$Creat_Corr_Perch
sum_weighted_perch_creat <- sum(weighted_perch_creat)
sum_weights_creat <- sum(adult_data_NHANES_PERCH_CREAT$WTSA2YR)

weighted_mean_perch_creat <- sum_weighted_perch_creat/sum_weights_creat #3.3381e-06

weighted_variance_perch_creat <- sum(adult_data_NHANES_PERCH_CREAT$WTSA2YR * (adult_data_NHANES_PERCH_CREAT$URXUP8 - weighted_mean_perch_creat)^2) / sum_weights_creat
weighted_std_dev_perch_creat <- sqrt(weighted_variance_perch_creat) #6.692114

#geomean = exp(sum(weights*log(perch))/sum(weights))
geomean_creat <- exp(sum(adult_data_NHANES_PERCH_CREAT$WTSA2YR*log(adult_data_NHANES_PERCH_CREAT$Creat_Corr_Perch))/sum(adult_data_NHANES_PERCH_CREAT$WTSA2YR)) #2.326121e-06

#geoSD: 
sqrt(exp(sum(adult_data_NHANES_PERCH_CREAT$WTSA2YR*((log(adult_data_NHANES_PERCH_CREAT$Creat_Corr_Perch)-log(geomean_creat))^2))/sum(adult_data_NHANES_PERCH_CREAT$WTSA2YR))) #1.3287
...
#I need to do with with the logtransformed data!
#wtd.t.test(x=log(All_Data$Result_P_ng_mL), y=log(adult_data_NHANES$URXUP8), weighty = adult_data_NHANES_PERCH$WTSA2YR, samedata = F) #p=0.516, two sample test - we want one sample

weighted_geomean_perch <- weighted.mean(log(adult_data_NHANES_PERCH$URXUP8), 
                                    w = adult_data_NHANES_PERCH$WTSA2YR, 
                                    na.rm = TRUE)

t.test(x=log(All_Data$Result_P_ng_mL), mu=weighted_geomean_perch, alternative="two.sided", conf.level = 0.95) #p=0.4779


#Creatinine

#URXUCR units mg/dL
weighted_geomean_creat <- weighted.mean(log(adult_data_NHANES_CREAT$URXUCR), 
                                    w = adult_data_NHANES_CREAT$WTSA2YR, 
                                    na.rm = TRUE)

t.test(x=log(All_Data$Result_C_mg_dL), mu=weighted_geomean_creat, alternative="two.sided", conf.level = 0.95) #p=0.1483

#Finally for creatinine-corrected perchlorate:
weighted_geomean_perch_creat <- weighted.mean(log(adult_data_NHANES_PERCH_CREAT$Creat_Corr_Perch), 
                                    w = adult_data_NHANES_PERCH_CREAT$WTSA2YR, 
                                    na.rm = TRUE)

t.test(x=log(All_Data$Result_P_ng_mL/(All_Data$Result_C_mg_dL*10000)), mu=weighted_geomean_perch_creat, alternative="two.sided") #p-value = 0.01237

#Hispanic Subpopulation

adult_data_NHANES_PERCH_Hisp <- subset(adult_data_NHANES_PERCH, adult_data_NHANES_PERCH$Ethnicity==1) #416 observations

adult_data_NHANES_CREAT_Hisp <- subset(adult_data_NHANES_CREAT, adult_data_NHANES_CREAT$Ethnicity==1) #418 observations

adult_data_NHANES_PERCH_CREAT_Hisp <- subset(adult_data_NHANES_PERCH_CREAT, adult_data_NHANES_PERCH_CREAT$Ethnicity==1) #415 observations


#Going to do manually (n=416)
weighted_perch <- adult_data_NHANES_PERCH_Hisp$WTSA2YR*adult_data_NHANES_PERCH_Hisp$URXUP8
sum_weighted_perch <- sum(weighted_perch)
sum_weights <- sum(adult_data_NHANES_PERCH_Hisp$WTSA2YR)

weighted_mean <- sum_weighted_perch/sum_weights #3.231

weighted_variance <- sum(adult_data_NHANES_PERCH_Hisp$WTSA2YR * (adult_data_NHANES_PERCH_Hisp$URXUP8 - weighted_mean)^2) / sum_weights
weighted_std_dev <- sqrt(weighted_variance) #6.026

#Median: 
sorted_NHANES_Perch <- adult_data_NHANES_PERCH_Hisp[order(adult_data_NHANES_PERCH_Hisp$URXUP8), ]
median(sorted_NHANES_Perch$URXUP8) #2.14
...
# The weighted median is the URXUP8 value at this index
weighted_median <- sorted_NHANES_Perch$URXUP8[median_index] #2.14


#geomean = exp(sum(weights*log(perch))/sum(weights))
geomean <- exp(sum(adult_data_NHANES_PERCH_Hisp$WTSA2YR*log(adult_data_NHANES_PERCH_Hisp$URXUP8))/sum(adult_data_NHANES_PERCH_Hisp$WTSA2YR)) #2.103#############

#geoSD: 
sqrt(exp(sum(adult_data_NHANES_PERCH_Hisp$WTSA2YR * ((log(adult_data_NHANES_PERCH_Hisp$URXUP8) - log(geomean))^2)) / sum(adult_data_NHANES_PERCH_Hisp$WTSA2YR))) #1.461##############

#Creatinine

#Going to do manually (n=418) for creatinine corrected perch.
weighted_creat <- adult_data_NHANES_CREAT_Hisp$WTSA2YR*adult_data_NHANES_CREAT_Hisp$URXUCR
sum_weighted_creat <- sum(weighted_creat)
sum_weights_creat <- sum(adult_data_NHANES_CREAT_Hisp$WTSA2YR)

weighted_mean_creat <- sum_weighted_creat/sum_weights_creat #125.41

weighted_variance_creat <- sum(adult_data_NHANES_CREAT_Hisp$WTSA2YR * (adult_data_NHANES_CREAT_Hisp$URXUCR - weighted_mean_creat)^2) / sum_weights_creat
weighted_std_dev_creat <- sqrt(weighted_variance_creat) #77.79

#geomean = exp(sum(weights*log(perch))/sum(weights))
geomean_creat <- exp(sum(adult_data_NHANES_CREAT_Hisp$WTSA2YR*log(adult_data_NHANES_CREAT_Hisp$URXUCR))/sum(adult_data_NHANES_CREAT_Hisp$WTSA2YR)) #100.59###########

#geoSD: 
sqrt(exp(sum(adult_data_NHANES_CREAT_Hisp$WTSA2YR*((log(adult_data_NHANES_CREAT_Hisp$URXUCR)-log(geomean_creat))^2))/sum(adult_data_NHANES_CREAT_Hisp$WTSA2YR))) #1.297########


#Going to do manually (n=415) for creatinine corrected perch.
weighted_perch_creat <- adult_data_NHANES_PERCH_CREAT_Hisp$WTSA2YR*adult_data_NHANES_PERCH_CREAT_Hisp$Creat_Corr_Perch
sum_weighted_perch_creat <- sum(weighted_perch_creat)
sum_weights_creat <- sum(adult_data_NHANES_PERCH_CREAT_Hisp$WTSA2YR)

weighted_mean_perch_creat <- sum_weighted_perch_creat/sum_weights_creat #2.91809e-06

weighted_variance_perch_creat <- sum(adult_data_NHANES_PERCH_CREAT_Hisp$WTSA2YR * (adult_data_NHANES_PERCH_CREAT_Hisp$URXUP8 - weighted_mean_perch_creat)^2) / sum_weights_creat
weighted_std_dev_perch_creat <- sqrt(weighted_variance_perch_creat) #6.846

#geomean = exp(sum(weights*log(perch))/sum(weights))
geomean_perch_creat <- exp(sum(adult_data_NHANES_PERCH_CREAT_Hisp$WTSA2YR*log(adult_data_NHANES_PERCH_CREAT_Hisp$Creat_Corr_Perch))/sum(adult_data_NHANES_PERCH_CREAT_Hisp$WTSA2YR)) #2.1095-06

#geoSD: 
sqrt(exp(sum(adult_data_NHANES_PERCH_CREAT_Hisp$WTSA2YR*((log(adult_data_NHANES_PERCH_CREAT_Hisp$Creat_Corr_Perch)-log(geomean_perch_creat))^2))/sum(adult_data_NHANES_PERCH_CREAT_Hisp$WTSA2YR))) #1.3229
...
hline_data3_M <- data.frame(
  yintercept = c(2.11, 2.52), 
  label = c("Hispanic/Latino\nWeighted National\nAverage", "Hispanic/Latino\nSample Average"),
  color = c("firebrick", "forestgreen")
)
...
ggplot(Sampled_All_Data, aes(y = Result_P_ug_mL/Result_C_g_mL, x=as.factor(Site))) +
  ...
  scale_linetype_manual(name = "",
                        values = c(2, 5),
                        labels = c("Hispanic/Latino\nSample Average","Hispanic/Latino\nWeighted National\nAverage"))+
  scale_color_manual(name = "",
                     values = setNames(hline_data3_M$color, hline_data3_M$label))+
  guides(linetype = guide_legend(title = ""),
         color = guide_legend(title = ""))+
...
#Stats tests for weighted values...
#I need to do with with the logtransformed data!
weighted_geomean_perch <- weighted.mean(log(adult_data_NHANES_PERCH_Hisp$URXUP8), 
                                    w = adult_data_NHANES_PERCH_Hisp$WTSA2YR, 
                                    na.rm = TRUE)

t.test(x=log(Sampled_All_Data$Result_P_ng_mL), mu=weighted_geomean_perch, alternative="two.sided", conf.level = 0.95) #p=0.05942


#Creatinine

#URXUCR units mg/dL
weighted_geomean_creat <- weighted.mean(log(adult_data_NHANES_CREAT_Hisp$URXUCR), 
                                    w = adult_data_NHANES_CREAT_Hisp$WTSA2YR, 
                                    na.rm = TRUE)

t.test(x=log(Sampled_All_Data$Result_C_mg_dL), mu=weighted_geomean_creat, alternative="two.sided", conf.level = 0.95) #p=0.1147

#Finally for creatinine-corrected perchlorate:
weighted_geomean_perch_creat <- weighted.mean(log(adult_data_NHANES_PERCH_CREAT_Hisp$Creat_Corr_Perch), 
                                    w = adult_data_NHANES_PERCH_CREAT_Hisp$WTSA2YR, 
                                    na.rm = TRUE)

t.test(x=log(Sampled_All_Data$Result_P_ng_mL/(All_Data$Result_C_mg_dL*10000)), mu=weighted_geomean_perch_creat, alternative="two.sided") #p-value = 0.01237
...
All_Data$Current_Smoker_D <- ifelse(All_Data$Smoking_Status==0 | 
                               ...
                               ifelse(All_Data$Smoking_Status == "2" | 
                                        All_Data$Smoking_Status == "3", 1, NA)) #Smoking is irrelevent. Pulled from EMRs by another researcher, not included in survey.
...
#Checking referent categories
levels(as.factor(All_Data$Site))
All_Data$Site <- C(All_Data$Site, contr.treatment, base=1)
levels(as.factor(All_Data$Water_all))
levels(as.factor(All_Data$Ethnicity))

model_hypo_crude <- glm(Hypo_D ~ log(Result_P_ng_mL) + Gender + age + Ethnicity, data=(All_Data), family="binomial") #General model with matched variables included for residual confounding
summary(model_hypo_crude)
exp(confint(model_hypo_crude))

model_hypo_adjusted_F <- glm(Hypo_D ~ log(Result_P_ng_mL) + log(Result_C_mg_dL) + Gender + age + Ethnicity + as.factor(Site)  + as.factor(Water_all_revised), data=(All_Data), family="binomial") #adjusted for creatinine, site, water
summary(model_hypo_adjusted_F)
exp(confint(model_hypo_adjusted_F))
...
All_Data %>% 
  ...
  filter(!is.na(Hypo_D)) %>% 
  summarise('cases'=sum(Hypo_D==1),
            'controls'=sum(Hypo_D==0))
```